                    ...
                    <div>
                        ...
                        <table class="preview-vacancy-table">
                            ...
                            <tbody>
                                ...
                                `).join('') : `
                                    <tr><td colspan="7" style="padding:20px; color:#94a3b8;">공실 없음</td></tr>
                                `}
...
function getLeasingGuideVacancies(buildingId) {
    ...
    const seen = new Set();
    vacancies.forEach(v => {
        const key = `${v.floor}_${v.area || v.rentArea}`;
        if (!seen.has(key)) {
            seen.add(key);
...
export function toggleGuideVacancy(idx, vacancyIdx) {
    ...
    const existingIdx = item.leasingGuideVacancies.findIndex(v => v.floor === targetVacancy.floor);
