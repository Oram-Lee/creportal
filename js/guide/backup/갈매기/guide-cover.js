...
import { state, DEFAULT_REGIONS, getAllRegions, addCustomRegion, removeCustomRegion, setEndingSettings, saveSettingsToLocal, loadSettingsFromLocal } from './guide-state.js';
        ...
        <div class="cover-editor">
            ...
            <div class="cover-setting-group">
                ...
                <div style="display:flex; gap:16px; align-items:center;">
                    ...
                    <div class="logo-position-selector">
                        <span style="font-size:12px; color:var(--text-muted); margin-bottom:6px; display:block;">로고 위치</span>
                        <div class="position-btn-group">
                            <button class="position-btn ${cs.logoPosition === 'left' ? 'active' : ''}" onclick="setLogoPosition('left')">◀ 왼쪽</button>
                            <button class="position-btn ${cs.logoPosition === 'center' ? 'active' : ''}" onclick="setLogoPosition('center')">● 중앙</button>
                            <button class="position-btn ${cs.logoPosition === 'right' ? 'active' : ''}" onclick="setLogoPosition('right')">오른쪽 ▶</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="cover-setting-group">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:12px;">
                    <div>
                        <div style="font-size:11px; color:var(--text-muted); margin-bottom:4px;">메인 타이틀</div>
                    ...
                    <div>
                        <div style="font-size:11px; color:var(--text-muted); margin-bottom:4px;">서브 타이틀</div>
                    ...
                    <div>
                        <div style="font-size:11px; color:var(--text-muted); margin-bottom:4px;">슬로건</div>
        ...
        <div class="modal-overlay show" id="regionManagerModal" onclick="if(event.target===this)closeRegionManager()">
            <div class="modal" style="max-width:700px;">
                ...
                <div class="modal-body">
                    <!-- 새 권역 추가 폼 (항상 표시) -->
                    <div class="region-add-form-always">
                        <h4>➕ 새 권역 추가</h4>
                    ...
                    <div class="region-section">
                        <h4>✏️ 커스텀 권역 (${state.customRegions.length}개)</h4>
                        <div class="region-list" id="customRegionList">
                            ${state.customRegions.length === 0 ? 
                                '<div class="empty-state" style="padding:20px; text-align:center; color:var(--text-muted);">추가된 커스텀 권역이 없습니다.<br>위 폼에서 새 권역을 추가하세요.</div>' :
                                state.customRegions.map(r => `
                                    <div class="region-item custom">
                                        <span class="region-code">${r.code}</span>
                                        <span class="region-name">${r.name}</span>
                                        <span class="region-name-en">${r.nameEn || ''}</span>
                                        <button class="btn btn-xs btn-danger" onclick="deleteCustomRegion('${r.code}')">삭제</button>
                                    </div>
                                `).join('')
                            }
                        </div>
