...
window.__SC_BUILD = 'DEPLOY-REALIGN-20260706-E';   // 재배포 트리거용 스탬프 (매 배포 시 갱신)
    ...
    wrap.innerHTML = `
        ...
        <div style="padding:16px 24px; background:linear-gradient(135deg,#0f4c81,#1a73e8);
            ...
            <div style="display:flex; align-items:center; gap:10px;">
                <button onclick="window._scSnapCapture && window._scSnapCapture()"
                    style="padding:6px 12px; background:rgba(255,255,255,0.28);
                           border:1px solid rgba(255,255,255,0.45); color:#fff;
                           border-radius:7px; cursor:pointer; font-size:12px; font-weight:700;"
                    title="현재 선택된 빌딩·공실을 스냅샷으로 잡아 여러 시점에 적용">
                    📌 현재 선택으로 다시점
                </button>
                <button onclick="window._scOpenStatsTrend && window._scOpenStatsTrend()"
...
// ═══════════════════════════════════════════════════════════════
// 7. 다시점 공실률 추이 (Multi-Period Vacancy Trend) — 독립 모달
//    자동 집계: 큐레이션 없이 _scVacanciesByMonth + _scVacAreaPy + _scGrossPy 재사용.
//    산식 동일: 공실률 = Σ공실면적(임대평) ÷ Σ연면적 × 100 (권역 가중)
//    형식 통일: srNormalizeDate 사용 → '25-10' / '2026-02' 혼재 자동 병합.
// ═══════════════════════════════════════════════════════════════

const _scTrendState = {
    months: [],          // 선택 시점 (가용 순서=오름차순 유지)
    mode:  'reported',   // 'reported'(보고된 빌딩 기준) | 'common'(공통 빌딩 고정)
};

function _scTrendNormBuildings() {
    return (window.srLib?.srGetNormBuildings?.() || srGetNormBuildings()) || [];
}
/** 가용 시점 (정규화·오름차순) */
function _scTrendAvailableMonths() {
    return srGetAllPublishDates(_scTrendNormBuildings()) || [];
}
/** 빌딩 유니버스 — 연면적>0 정규화 빌딩 */
function _scTrendUniverse() {
    return _scTrendNormBuildings().filter(b => _scGrossPy(b) > 0);
}
/** 한 빌딩·한 시점 공실면적(평) — 층+면적 시그니처로 크로스소스 중복 제거 */
function _scTrendVacArea(b, yyyymm) {
    const vacs = _scVacanciesByMonth(b, yyyymm);
    if (!vacs.length) return { area: 0, hasData: false };
    const seen = new Set();
    let area = 0;
    vacs.forEach(v => {
        const sig = String(v.floorText || v.floor || '').trim() + '__' + _scVacAreaPy(v);
        if (seen.has(sig)) return;
        seen.add(sig);
        area += _scVacAreaPy(v);
    });
    return { area, hasData: true };
}
/** 한 시점 권역별 집계.
 *  requireData=true  → 그 달 공실 보고가 있는 빌딩만 (분모=그 빌딩들 Σ연면적)  [reported]
 *  requireData=false → 전달 빌딩 전체 (없으면 0%, 분모=전체 Σ연면적)            [common]
 *  @returns Map<region,{gross,vac,rate,nBldg}>
 */
function _scTrendAggMonth(yyyymm, buildings, requireData) {
    ...
    buildings.forEach(b => {
        const gross = _scGrossPy(b);
        if (gross <= 0) return;
        const { area, hasData } = _scTrendVacArea(b, yyyymm);
        if (requireData && !hasData) return;
        const region = regOf(b._region || 'ETC');
        [map.get(region), map.get('TOTAL')].forEach(bk => {
            bk.gross += gross; bk.vac += area; bk.nBldg += 1;
        });
    });
    map.forEach(bk => { bk.rate = bk.gross > 0 ? (bk.vac / bk.gross * 100) : 0; });
    return map;
}
/** 공통 빌딩(선택 시점 전부에 공실 데이터 보유) — Map<id,building> */
function _scTrendCommonSet(months, universe) {
    const out = new Map();
    if (!months.length) return out;
    universe.forEach(b => {
        let all = true;
        for (const m of months) { if (!_scVacanciesByMonth(b, m).length) { all = false; break; } }
        if (all) out.set(String(b.id), b);
    });
    return out;
}

/** 권역별 라인차트 (인라인 SVG, PPT 캡쳐용 정적) */
function _scTrendChartSVG(perMonth, regions, mode) {
    const W = 1180, H = 440, padL = 56, padR = 150, padT = 26, padB = 56;
    const plotW = W - padL - padR, plotH = H - padT - padB;
    const n = perMonth.length;
    const xs = i => padL + (n <= 1 ? plotW / 2 : plotW * i / (n - 1));
    let maxRate = 0;
    perMonth.forEach(pm => regions.forEach(r => { maxRate = Math.max(maxRate, pm.map.get(r)?.rate || 0); }));
    const yMax = Math.max(1, Math.ceil(maxRate * 1.15));
    const ys = v => padT + plotH - (plotH * v / yMax);
    const colOf = r => (r === 'TOTAL' ? '#0f172a' : (SR_REGION_COLOR[r] || '#94a3b8'));

    let grid = '';
    const ticks = 5;
    for (let t = 0; t <= ticks; t++) {
        const v = yMax * t / ticks, y = ys(v);
        grid += `<line x1="${padL}" y1="${y.toFixed(1)}" x2="${padL + plotW}" y2="${y.toFixed(1)}" stroke="#eef2f7" stroke-width="1"/>`
             +  `<text x="${padL - 8}" y="${(y + 3).toFixed(1)}" text-anchor="end" font-size="10" fill="#94a3b8">${v.toFixed(1)}%</text>`;
    }
    let xlab = '';
    perMonth.forEach((pm, i) => {
        xlab += `<text x="${xs(i).toFixed(1)}" y="${padT + plotH + 18}" text-anchor="middle" font-size="10" fill="#64748b">${pm.month}</text>`;
        if (mode === 'reported') {
            xlab += `<text x="${xs(i).toFixed(1)}" y="${padT + plotH + 32}" text-anchor="middle" font-size="8" fill="#cbd5e1">n=${pm.map.get('TOTAL')?.nBldg ?? 0}</text>`;
        }
    });
    let lines = '', legend = '';
    regions.forEach((r, ri) => {
        const pts = perMonth.map((pm, i) => ({ x: xs(i), y: ys(pm.map.get(r)?.rate || 0), v: pm.map.get(r)?.rate || 0 }));
        const d = pts.map((p, i) => `${i === 0 ? 'M' : 'L'}${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' ');
        const c = colOf(r), isTotal = (r === 'TOTAL');
        lines += `<path d="${d}" fill="none" stroke="${c}" stroke-width="${isTotal ? 3 : 2}" opacity="${isTotal ? 1 : 0.88}"/>`;
        pts.forEach(p => { lines += `<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="${isTotal ? 3.6 : 2.8}" fill="${c}"/>`; });
        if (isTotal) pts.forEach(p => { lines += `<text x="${p.x.toFixed(1)}" y="${(p.y - 9).toFixed(1)}" text-anchor="middle" font-size="9" font-weight="700" fill="${c}">${p.v.toFixed(1)}</text>`; });
        const ly = padT + 8 + ri * 19;
        legend += `<line x1="${padL + plotW + 18}" y1="${ly}" x2="${padL + plotW + 38}" y2="${ly}" stroke="${c}" stroke-width="${isTotal ? 3 : 2}"/>`
               +  `<text x="${padL + plotW + 44}" y="${ly + 3}" font-size="11" font-weight="${isTotal ? 700 : 500}" fill="${isTotal ? '#0f172a' : '#475569'}">${isTotal ? '전체' : r}</text>`;
    });
    return `<div style="width:100%; overflow-x:auto;">
        <svg viewBox="0 0 ${W} ${H}" style="width:100%; min-width:780px; background:#fff; border:1px solid #eef2f7; border-radius:10px;">
            <text x="${padL}" y="17" font-size="12" font-weight="700" fill="#0f172a">📈 권역별 공실률 추이 (%)</text>
            ${grid}
...
/** 시점 × 권역 표 (시점간 증감 포함) */
function _scTrendTable(perMonth, regions) {
    const head = `<th style="padding:6px 8px; text-align:left; position:sticky; left:0; background:#f8fafc;">권역</th>`
        + perMonth.map(pm => `<th style="padding:6px 8px; text-align:right; white-space:nowrap;">${pm.month}</th>`).join('');
    const body = regions.map(r => {
        const isTotal = (r === 'TOTAL');
        const c = isTotal ? '#0f172a' : (SR_REGION_COLOR[r] || '#64748b');
        const cells = perMonth.map((pm, i) => {
            const cur = pm.map.get(r)?.rate;
            const prev = i > 0 ? perMonth[i - 1].map.get(r)?.rate : null;
            let delta = '';
            if (prev != null && cur != null) {
                const dd = cur - prev;
                const dc = Math.abs(dd) < 0.05 ? '#9ca3af' : (dd > 0 ? '#dc2626' : '#16a34a');
                delta = `<div style="font-size:9px; color:${dc};">${dd > 0 ? '▲' : (dd < 0 ? '▼' : '–')}${Math.abs(dd).toFixed(2)}p</div>`;
            }
            return `<td style="padding:5px 8px; text-align:right;"><div style="font-weight:${isTotal ? 700 : 500}; color:${c};">${cur != null ? cur.toFixed(2) : '-'}%</div>${delta}</td>`;
        }).join('');
        return `<tr style="border-top:1px solid var(--border-color); ${isTotal ? 'background:#f1f5f9;' : ''}">`
            + `<td style="padding:5px 8px; font-weight:700; color:${c}; position:sticky; left:0; background:${isTotal ? '#f1f5f9' : '#fff'};">${isTotal ? '🌐 전체' : r}</td>${cells}</tr>`;
    }).join('');
    const nrow = `<tr style="border-top:2px solid #e5e7eb;"><td style="padding:5px 8px; font-size:10px; color:#94a3b8; position:sticky; left:0; background:#fff;">빌딩수(n)</td>`
        + perMonth.map(pm => `<td style="padding:5px 8px; text-align:right; font-size:10px; color:#94a3b8;">${pm.map.get('TOTAL')?.nBldg ?? '-'}</td>`).join('') + `</tr>`;
    return `<div style="width:100%; overflow-x:auto;"><table style="width:100%; border-collapse:collapse; font-size:12px;">
        <thead><tr style="background:#f8fafc; color:#475569;">${head}</tr></thead>
        <tbody>${body}${nrow}</tbody></table></div>`;
}

window._scTrendRender = function () {
    const months = _scTrendState.months;
    const area = _scQS('sc-trend-result');
    if (!area) return;
    if (!months || months.length < 2) {
        area.innerHTML = `<div style="padding:40px; text-align:center; color:#94a3b8;">시점을 2개 이상 선택한 뒤 [추이 계산]을 누르세요.</div>`;
        return;
    }
    const universe = _scTrendUniverse();
    let perMonth, commonCount = null;
    if (_scTrendState.mode === 'common') {
        const commonArr = [..._scTrendCommonSet(months, universe).values()];
        commonCount = commonArr.length;
        perMonth = months.map(m => ({ month: m, map: _scTrendAggMonth(m, commonArr, false) }));
    } else {
        perMonth = months.map(m => ({ month: m, map: _scTrendAggMonth(m, universe, true) }));
    }
    const present = SR_REGIONS.filter(r => perMonth.some(pm => (pm.map.get(r)?.gross || 0) > 0));
    const regionsToPlot = ['TOTAL', ...present];
    const svg = _scTrendChartSVG(perMonth, regionsToPlot, _scTrendState.mode);
    const table = _scTrendTable(perMonth, regionsToPlot);
    const note = _scTrendState.mode === 'common'
        ? `<b>공통 빌딩 고정</b> — 선택한 ${months.length}개 시점 전부에 공실 데이터가 있는 <b>${commonCount || 0}개</b> 빌딩만으로 계산(분모 일정, 엄밀 비교).`
        : `<b>보고된 빌딩 기준</b> — 각 시점에 공실이 보고된 빌딩들의 면적가중 공실률. 시점마다 빌딩수(n)가 달라 추이 해석 시 참고.`;
    area.innerHTML = `
        <div style="padding:4px 2px 14px; font-size:12px; color:#64748b;">ℹ️ ${note}</div>
        ${svg}
        <div style="margin-top:18px;">${table}</div>
        <div style="margin-top:10px; font-size:11px; color:#94a3b8; line-height:1.6;">
            산식: 공실률 = Σ공실면적(임대평) ÷ Σ연면적 × 100 (권역 가중) · 같은 층을 여러 회사가 게재한 경우 층+면적으로 1건 처리 · 자동 집계라 수기 큐레이션 2시점 비교와 미세하게 다를 수 있습니다.
        </div>`;
};

function _scInjectTrendModal() {
    if (_scQS('sc-trend-modal')) return;
    const wrap = document.createElement('div');
    wrap.id = 'sc-trend-modal';
    wrap.style.cssText = `display:none; position:fixed; top:50%; left:50%; transform:translate(-50%,-50%);
        background:var(--bg-card); border-radius:14px; width:97%; max-width:1280px; z-index:1030;
    ...
    _scTrendState.months = months.slice();   // 기본 전체 선택
    const chk = months.map(m => `
        <label style="display:inline-flex; align-items:center; gap:4px; font-size:12px; padding:3px 8px; background:#f1f5f9; border-radius:6px; cursor:pointer;">
            <input type="checkbox" value="${m}" checked onchange="window._scTrendToggleMonth(this)"> ${m}
        </label>`).join('');
    wrap.innerHTML = `
        <div style="padding:16px 24px; background:linear-gradient(135deg,#0f4c81,#1a73e8); border-radius:14px 14px 0 0; display:flex; justify-content:space-between; align-items:center; flex-shrink:0;">
            <div>
                <div style="font-size:16px; font-weight:700; color:#fff;">📈 다시점 공실률 추이</div>
                <div style="font-size:12px; color:rgba(255,255,255,0.8); margin-top:2px;">권역별 공실률을 여러 시점에 걸쳐 자동 집계 — 추이·움직임 분석</div>
            </div>
            <button onclick="window._scCloseTrend()" style="background:rgba(255,255,255,0.15); border:none; color:#fff; font-size:20px; width:34px; height:34px; border-radius:8px; cursor:pointer;">×</button>
        </div>
        <div style="overflow-y:auto; flex:1; min-height:0; padding:16px 24px;">
            <div style="display:flex; flex-wrap:wrap; gap:14px; align-items:flex-start; padding:12px 14px; background:#f8fafc; border:1px solid #e5e7eb; border-radius:10px; margin-bottom:16px;">
                <div style="display:flex; align-items:center; gap:6px;">
                    <span style="font-size:12px; font-weight:700; color:#475569;">기준</span>
                    <select id="sc-trend-mode" onchange="window._scTrendSetMode(this.value)" style="font-size:12px; padding:5px 8px; border:1px solid #cbd5e1; border-radius:6px;">
                        <option value="reported">보고된 빌딩 기준 (시점별)</option>
                        <option value="common">공통 빌딩 고정</option>
                    </select>
                </div>
                <div style="flex:1; min-width:300px;">
                    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:5px;">
                        <span style="font-size:12px; font-weight:700; color:#475569;">시점 선택</span>
                        <span>
                            <button onclick="window._scTrendSelectAll(true)" style="font-size:11px; padding:2px 8px; border:1px solid #cbd5e1; background:#fff; border-radius:5px; cursor:pointer;">전체</button>
                            <button onclick="window._scTrendSelectAll(false)" style="font-size:11px; padding:2px 8px; border:1px solid #cbd5e1; background:#fff; border-radius:5px; cursor:pointer;">해제</button>
                        </span>
                    </div>
                    <div style="display:flex; flex-wrap:wrap; gap:6px;">${chk || '<span style="font-size:12px;color:#94a3b8;">사용 가능한 시점이 없습니다.</span>'}</div>
                </div>
                <button onclick="window._scTrendRender()" style="padding:8px 16px; background:#1a73e8; color:#fff; border:none; border-radius:7px; font-size:13px; font-weight:700; cursor:pointer; white-space:nowrap; align-self:center;">📊 추이 계산</button>
            </div>
            <div id="sc-trend-result"></div>
        </div>`;
    document.body.appendChild(wrap);
}

window._scOpenStatsTrend = function () {
    _scInjectTrendModal();
    const m = _scQS('sc-trend-modal');
    if (m) m.style.display = 'flex';
    window._scTrendRender();
};
window._scCloseTrend = function () { const m = _scQS('sc-trend-modal'); if (m) m.style.display = 'none'; };
window._scTrendToggleMonth = function (cb) {
    const avail = _scTrendAvailableMonths();
    const set = new Set(_scTrendState.months);
    if (cb.checked) set.add(cb.value); else set.delete(cb.value);
    _scTrendState.months = avail.filter(m => set.has(m));   // 오름차순 유지
};
window._scTrendSelectAll = function (on) {
    document.querySelectorAll('#sc-trend-modal input[type=checkbox]').forEach(cb => { cb.checked = on; });
    _scTrendState.months = on ? _scTrendAvailableMonths().slice() : [];
    window._scTrendRender();
};
window._scTrendSetMode = function (v) { _scTrendState.mode = v; window._scTrendRender(); };

// ═══════════════════════════════════════════════════════════════
// 8. 선택 기반 다시점 세션 (Snapshot Trend) — 수동 큐레이션을 N시점으로
//    캡쳐: 현재 A/B 선택 → 빌딩별 (층+면적) 시그니처 스냅샷
//    적용: 임의 시점에 시그니처 매칭 → 공실면적
//          · 해소(그 시점에 없음) = 0
//          · 신규(스냅샷에 없던 공실 출현) = 묻고 선택 포함
//          · 지하·1층(리테일) = 계산 제외
...
function _scInjectSnapModal() {
    ...
    if (old) old.remove();   // 캡쳐 시마다 새로 (스냅샷 요약·시점 갱신)
    const wrap = document.createElement('div');
    wrap.id = 'sc-snap-modal';
    wrap.style.cssText = `display:none; position:fixed; top:50%; left:50%; transform:translate(-50%,-50%);
        ...
        <div style="overflow-y:auto; flex:1; min-height:0; padding:16px 24px;">
            <div style="display:flex; flex-wrap:wrap; gap:14px; align-items:flex-start; padding:12px 14px; background:#f8fafc; border:1px solid #e5e7eb; border-radius:10px; margin-bottom:16px;">
                <div style="font-size:12px; color:#475569;">
                    <div style="font-weight:700;">스냅샷</div>
                    <div style="margin-top:2px;">${_scSnapState.snapshot.size}개 빌딩</div>
                </div>
                <div style="flex:1; min-width:300px;">
                    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:5px;">
                        <span style="font-size:12px; font-weight:700; color:#475569;">시점 선택</span>
                        <span>
                            <button onclick="window._scSnapSelectAll(true)" style="font-size:11px; padding:2px 8px; border:1px solid #cbd5e1; background:#fff; border-radius:5px; cursor:pointer;">전체</button>
                            <button onclick="window._scSnapSelectAll(false)" style="font-size:11px; padding:2px 8px; border:1px solid #cbd5e1; background:#fff; border-radius:5px; cursor:pointer;">해제</button>
                        </span>
                    </div>
                    <div style="display:flex; flex-wrap:wrap; gap:6px;">${chk || '<span style="font-size:12px;color:#94a3b8;">사용 가능한 시점이 없습니다.</span>'}</div>
                </div>
