 ...
 *
 * Phase 6 Post-Tier B 핫픽스 (2026-04): 4번째 인자로 _seState.quarter 전달.
 *   portal-stats.js 의 _srPersistentExclude.loadedQuarter 에 기록되어
 *   뷰어가 다른 분기로 이동할 때 자동 재로드를 트리거하는 키로 사용됨.
 */
 ...
 *
 * Phase 6 Post-Tier B 핫픽스 (2026-04):
 *   · 기본 분기를 "오늘 날짜 기준 분기" → "직전 완료 분기" 로 변경 (편집 모달과 동일 로직).
 *     이유: 오늘 기준 분기는 진행중 분기라 편집 스냅샷이 없을 확률이 높아 빈 필터로 시작됨.
 *     직전 완료 분기는 통상 스냅샷이 존재하므로 유의미한 초기 상태를 제공.
 *   · 실제 로딩 로직은 portal-stats.js 의 _srLoadPersistentForQuarter 에 위임
 *     (중복 구현 제거 + loadedQuarter 추적 통합).
 */
window._seAutoLoadForStats = async function() {
    // 직전 완료 분기 산출 (편집 모달 _sePrevCompletedQuarter 와 동일 로직)
    const now   = new Date();
    const m     = now.getMonth() + 1;
    const y     = now.getFullYear();
    const curQ  = Math.ceil(m / 3);
    const prevQ = curQ === 1 ? 4 : curQ - 1;
    const prevY = curQ === 1 ? y - 1 : y;
    const quarter = `${prevY}Q${prevQ}`;

    if (typeof window._srLoadPersistentForQuarter === 'function') {
        // 신규 통합 로더에 위임 (Firebase 읽기 + loadedQuarter 기록 + 재렌더)
        await window._srLoadPersistentForQuarter(quarter);
        return;
    }

    // Fallback: 구 경로 (portal-stats.js 가 구버전인 경우 대비)
    try {
        const { db, ref, get } = await import('./portal-firebase.js');
        const snap = await get(ref(db, `statsFilter/${quarter}`));
